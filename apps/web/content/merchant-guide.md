...
- **You never approve a sale by hand.** A sale turns **approved** by itself the moment its commission becomes payable, whether the holding period ran out or you clicked Pay now. Refunded, disputed and cancelled sales stay as they are. That is what "approved" counts in the Analytics funnel: sales that survived their holding period.
...
Analytics: clicks, sales, attributed revenue, commission and new affiliates over time with comparison to the previous period; the click → attributed → approved funnel (approved meaning the sale's holding period is over); breakdowns by affiliate, offer, program, campaign, group and attribution source. **Export data** (the button at the top of Analytics, or the section at the bottom of the page): pick what you need in plain words, the affiliate performance report for the period on screen, sales and leads, commissions, payouts, the ledger, affiliates or clicks as CSV, or the whole workspace as one JSON file (owners only), then pick the records: the period chosen at the top of the page, or everything. Each dataset says which date the period is measured on (a sale by the date of the sale, a click by the date of the click). Click **Prepare the file** and carry on working: the file is built in the background, a notice in the corner tells you when it is ready with a Download button in it, on whatever page you are by then, and on Analytics the file list scrolls into view with the new file highlighted. Files are named after what they hold and the period they cover, and stay downloadable for 7 days.
