...
export const tenants = pgTable("tenants", {
  ...
  /** When the snippet last reported, and from which host: the tracking page's "Receiving" status. Updated at most once a minute. */
  trackingLastAt: ts("tracking_last_at"),
  trackingLastHost: text("tracking_last_host"),
  createdAt: createdAt(),
  updatedAt: updatedAt(),
});
...
export interface TenantTracking {
  ...
  /** Address fragments that mark a checkout page ("/buy", "/enroll"), for sites whose checkout address does not contain "checkout". */
...
export const clicks = pgTable(
  "clicks",
  {
    ...
    /** The site snippet's first-party visitor id, noted when that visitor lands with this click: lets an order that carries the visitor id find the click after the ref is gone. */
    visitorId: text("visitor_id"),
    occurredAt: ts("occurred_at").notNull().defaultNow(),
  },
  (t) => [index("clicks_tenant_affiliate_idx").on(t.tenantId, t.affiliateId, t.occurredAt), index("clicks_tenant_visitor_idx").on(t.tenantId, t.visitorId)],
);
...
/**
 * Website journeys as totals (TRK-09). No visitor is recorded: the snippet reports each stage a
 * visitor reaches once a day, and this table only counts them, per day, affiliate, page and stage.
 * `page` is "" for the whole site, otherwise a normalised "host/path". Stages: visit, land (the
 * first page of the day), half, bottom, checkout, and `event:<name>` for the site's own events.
 * Purchases are not stored here: they are counted from the conversions themselves.
 */
export const journeyStats = pgTable(
  "journey_stats",
  {
    id: id(),
    tenantId: tenantRef(),
    /** UTC day the hits arrived on. */
  ...
  (t) => [uniqueIndex("journey_stats_uq").on(t.tenantId, t.day, t.affiliateId, t.page, t.stage), index("journey_stats_tenant_day_idx").on(t.tenantId, t.day)],
);
...
export type JourneyStat = typeof journeyStats.$inferSelect;
