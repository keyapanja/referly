...
- **Leads intake (pay-per-lead). Done (2026-09-09).** A lead is a conversion of kind `lead` with a contact record beside it (name, email, phone, company, extra form fields, landing page). Programs opt in with a fixed commission per qualified lead, manual or automatic qualification, and a dedupe window: a second lead with the same email for the same program inside the window is recorded as a duplicate and earns nothing. Attribution works exactly like sales (click token, coupon, or manual with a reason). Intake: `POST /v1/leads` for API keys with `conversions.write` (idempotent on `externalLeadId`), the Leads page for manual entry, and a public per-program capture endpoint `POST /capture/:token` for forms on the merchant's site (JSON or form-encoded, any origin, rate limited, optional redirect, forwards the click token from the landing page's `ref` parameter). Review queue with qualify (approve conversion and commission, holding period then settlement) and disqualify (cancel or reverse, commission voided or reversed). Leads never count as sales: analytics, tiers, campaign bonuses, plan revenue and the sales lists filter on kind. Affiliates see their leads in the portal with status and commission but no contact details. Notifications (merchant `leads` category, affiliate earnings) and a `lead_recorded` email/text template. Retention blanks contact details of settled leads after `leadsDays` (365 by default) while the record stays.
- **Website tracking snippet and journeys (TRK-09). Done (2026-09-09).** A first-party script merchants paste on their site (`GET /referly.js`, plain ES5, no dependencies) that keeps the click token from `?ref=` in a cookie for the program's attribution window, assigns a visitor id, reports page views (including SPA route changes) and custom events in batches to `POST /t/<siteKey>/events` (text/plain JSON, beacon-friendly), fills hidden `ref`/`referly_visitor` inputs in lead forms, and optionally reports orders from the thank-you page to `POST /t/<siteKey>/convert` (source `pixel`, idempotent on the order id, same attribution engine; off by default and only for a fixed domain list). Per-workspace site key (enable, rotate), domain list with subdomain matching, one 404 for unknown key, inactive workspace and foreign origin. Events resolve the affiliate through the click token or the visitor's earlier click; server-side conversions and leads accept `visitorId` as attribution evidence and every conversion lands on the visitor's journey through its id or its click. Merchant UI: Settings → Website tracking (snippet, domains, switch, activity, examples), Journeys page (visits from affiliate links, landing page, pages, events, outcome, filters, per-visitor timeline) and a journey card on the conversion page. Journey rows are RLS-scoped, backed up, exported, and pruned by retention (`journeyDays`, 90 by default).
- **Snippet consent mode (GDPR/ePrivacy). Done (2026-09-09).** Per-workspace switch that adds `data-consent="wait"` to the install snippet. In that mode the snippet writes no cookie, localStorage or sessionStorage entry and issues no request until the page reports a decision; events raised while the banner is open are held in a bounded in-memory queue and flushed on grant or discarded on decline, where any ids already in the browser are deleted and hidden visitor inputs blanked. Decisions arrive from `referly('consent', 'granted'|'denied')`, a `referly:consent` DOM event, or the Cookiebot and OneTrust callbacks, which are recognised without configuration; the decision is remembered for 180 days. Enforced server-side too: `/t/<key>/events` drops batches without `consent: "granted"` when the workspace requires it, and every journey row records the consent state it arrived under. Thank-you-page orders still record (contract data) but without the visitor id, so they attribute by click token or coupon only. Snippet version 2.
- **In-app guides per audience. Done (2026-09-09).** The three guides are markdown in `apps/web/content` and are the single source: `merchant-guide.md`, `affiliate-guide.md` (written for partners: links and codes, what breaks tracking, assets, campaigns, statuses, payouts, disputes, data) and `admin-guide.md` (workspaces, operations, backups, retention, triage). A dependency-free markdown subset parser renders them into React elements only, never HTML strings, so nothing in the content can inject markup; the pages are server components that read the file at build time (literal paths, so the bundler traces three files rather than the whole project) and are prerendered as static. Each guide gets a sticky numbered contents rail, a filter that hides non-matching sections, anchor links and print styles. Linked as **Guide** in the merchant, portal and admin sidebars. `docs/USER_GUIDE.md` is now a pointer to the three sources.
- **Website tracking page (TRK-09 follow-up). Done (2026-09-09).** Setup moved out of the Settings card, which was too cramped for it, onto its own page with the sidebar entry **Website tracking**: live status (receiving or waiting, reporting host, events and visitors over 7 days), a platform picker, the domain list, the order and consent switches, event and lead-form examples, and site-key rotation. The picker covers Custom website, WordPress, Shopify, Webflow, Wix or Squarespace, Google Tag Manager, and React/Next.js/single-page apps: each gives the click path for that platform and complete copy-paste code with the workspace's real site key already substituted, plus order reporting in the platform's own language (a WooCommerce PHP hook, Shopify Liquid on the order status page using its exact cents value, a Tag Manager purchase tag, a typed call for React). Built server-side in `apps/api/src/platforms.ts` and covered by tests that assert no placeholder ever reaches a merchant. The Settings card is now a summary that links here.
