...
One script tag on the merchant's site counts what affiliate visitors do there, in totals: `GET /referly.js` keeps the `?ref=` click token in a first-party cookie, assigns a visitor id (used only so an order can find its click), and reports the stages a person reaches (arrived, read half a page, read to the bottom, reached the checkout, plus the site's own named events) to `POST /t/<siteKey>/events` (text/plain JSON, beacon-friendly, any origin but checked against the workspace's domain list). Each stage is reported once per person per day, remembered in the visitor's own browser, so the server keeps daily counters per affiliate and page and no per-visitor rows. It also fills hidden inputs in lead forms, and can report orders from the thank-you page to `POST /t/<siteKey>/convert` when the workspace allows it (source `pixel`, same attribution and idempotency as every other sale). Server-side conversions may pass `visitorId` instead of a click token. Merchants see the funnel from arriving to buying, per page and per affiliate, under Journeys; purchases in it are counted from the recorded sales, not reported by the browser. WordPress sites skip the pasting: the Referly plugin, served by the API at `/wordpress/referly.zip`, takes one connection key from the Website tracking page, prints the tracking code and reports paid WooCommerce orders and refunds from the server through `/v1/conversions`, with a key that can do nothing else. Shopify and Stripe report orders through their own webhooks: the merchant pastes the signing secret on the Website tracking page and points the shop's or processor's webhooks at `/hooks/shopify/<tenant>` or `/hooks/stripe/<tenant>`; the API verifies each signature, maps paid orders, refunds and cancellations onto the same intake, logs every event with its outcome, and the snippet puts the click token where they hand it back (Shopify cart attributes, `client_reference_id` on Payment Links). Only affiliate traffic is tracked: the snippet stays silent for visitors without an affiliate click, and a sale no affiliate can claim is answered `recorded: false` and not stored. Workspaces on EU sites switch on consent mode: the snippet then stores and sends nothing until the page reports consent (Cookiebot and OneTrust recognised, or `referly('consent', 'granted')`), events during the banner are held in memory and flushed on grant or dropped on decline, and the API refuses unconsented batches.
